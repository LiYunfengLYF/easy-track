...
class Starkst(Starks):
    ...
    def forward_transformer(self, seq_dict, run_box_head=False, run_cls_head=False):
        ...
        output_embed, enc_mem = self.transformer(seq_dict["feat"], seq_dict["mask"], self.query_embed.weight,
                                                 seq_dict["pos"], return_encoder_output=True)
        # Forward the corner head
        out, outputs_coord = self.forward_head(output_embed, enc_mem, run_box_head=run_box_head,
...
def Starks50():
    ...
    model = Starks(backbone, transformer, box_head,
                   num_queries=cfg.num_object_queries,
                   aux_loss=cfg.deep_supervision,
...
def Starkst50():
    ...
    transformer = build_transformer(cfg)
    box_head = build_box_head(cfg)
    cls_head = MLP(cfg.hidden_dim, cfg.hidden_dim, 1, cfg.num_layer_head)
    model = Starkst(backbone, transformer, box_head,
                    num_queries=cfg.num_object_queries,
                    aux_loss=cfg.deep_supervision,
                    head_type=cfg.head_type,
                    cls_head=cls_head)
