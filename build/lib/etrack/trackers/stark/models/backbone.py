...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        if "resnet" in name:
            ...
            if name == 'resnet50':
                backbone = resnet50(replace_stride_with_dilation=[False, dilation, False],
                                    pretrained=is_main_process(), norm_layer=norm_layer, last_layer='layer3')
            elif name == 'resnet101':
                backbone = resnet101(replace_stride_with_dilation=[False, dilation, False],
                                     pretrained=is_main_process(), norm_layer=norm_layer, last_layer='layer3')
            else:
                raise print('only support resnet50 and resnet 101')
...
def build_backbone(cfg):
    ...
    train_backbone = cfg.backbone_multiplier > 0
    return_interm_layers = cfg.predict_mask

    ckpt_path = None
    backbone = Backbone(cfg.backbone_type, train_backbone, return_interm_layers,
                        cfg.dilation, cfg.freeze_backbone_bn, ckpt_path)
