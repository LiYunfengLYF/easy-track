...
from .fhog import getFeatureMaps, PCAFeatureMaps, normalizeAndTruncate
from .utils import fftd, real, rearrange, subwindow, complexMultiplication, extractImage, complexDivisionReal, \
    complexDivision
...
# KCF tracker
class kcf(Tracker):
    def __init__(self, hog: bool = True, fixed_window: bool = True):
        super().__init__(None, False, 'kcf_dsst')
        ...
        if self._multiscale:
            ...
            # if not hog:
            #     print('HOG feature is forced to turn off.')
        ...
        self._hogfeatures = True if hog or self._multiscale else False
    ...
    def init(self, image: np.array, roi: list) -> None:
    ...
    def get_scale_sample(self, image):
        ...
        for i in range(self.n_scales):
            ...
            mapp = {'sizeX': 0, 'sizeY': 0, 'numFeatures': 0, 'map': 0}
            mapp = getFeatureMaps(im_patch_resized, self.cell_size, mapp)
            mapp = normalizeAndTruncate(mapp, 0.2)
            mapp = PCAFeatureMaps(mapp)
