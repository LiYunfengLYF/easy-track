...
class transt(Tracker):
    def __init__(self, checkpoint_path=None, use_cuda=True):
        super().__init__(checkpoint_path, use_cuda, 'transt')
        self.cfg = cfg_n4()
        self.network = TransTN4().to(self.device).eval()
        self.load_checkpoint()

        self.window = np.outer(np.hanning(32), np.hanning(32)).flatten()
    ...
    def init(self, image: np.array, bbox: list) -> None:
        ...
        # get crop
        z_crop = self.get_subwindow(image, self.center_pos, self.cfg.track_exemplar_size, s_z, self.channel_average)
    ...
    def track(self, image: np.array) -> list:
        ...
        # window penalty
        pscore = score * (1 - self.cfg.track_window_influence) + self.window * self.cfg.track_window_influence
...
class transt_slt(Tracker):
    def __init__(self, checkpoint_path=None, use_cuda=True):
        super().__init__(checkpoint_path, use_cuda, 'transt_slt')
        self.cfg = cfg_n4()
        self.network = TransTN4().to(self.device).eval()
        self.load_checkpoint()
