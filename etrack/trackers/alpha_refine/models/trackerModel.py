...
from .neck import PtCorr
from .backbone import resnet34
from .corner_head import Corner_Predictor
from .mask_head import Mask_Predictor_fine
...
class SEcmnet(nn.Module):
    ...
    def forward_ref(self, train_imgs, train_bb):
        ...
        train_feat_dict = self.extract_backbone_features(
            train_imgs.view(-1, *train_imgs.shape[-3:]))  # `OrderedDict{'layer3':Tensor}`
...
def secm_resnet34(backbone_pretrained=True, used_layers=('layer3',), pool_size=8, unfreeze_layer3=False):
    # backbone
    backbone_net = resnet34(pretrained=backbone_pretrained)

    # neck
    neck_net = PtCorr(pool_size=pool_size)

    # multiple heads
    corner_head = Corner_Predictor(inplanes=pool_size * pool_size)  # 64
    mask_head = Mask_Predictor_fine()
